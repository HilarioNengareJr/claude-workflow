---
name: review
description: After building a feature, verify it matches what was planned, respects the system architecture and design standards, and is ready for production. Reports issues clearly so the developer decides what to fix. Use when the user says "review this", "is this ready to ship", "check what I built", "did this match the plan", or right after /build finishes. Scope is STATIC and plan-relative — it reads code against the plan and the standards. It does NOT run adversarial tests (/break), judge a decision or plan before it is built (/counsel), compare contracts across repos (/drift), or fix anything it finds.
---
...
- The implementation plan — the latest entry in `context/architecture.md` (written by `/architect`), if one exists
- The feature description or task that was given
- The binding standards: `context/code-standards.md` always; `context/ui-rules.md` → `context/ui-tokens.md` → `context/ui-registry.md`, in that order, when the feature touches UI
- `context/progress-tracker.md` — what was supposed to be in flight, so scope drift shows up in Layer 1
...
**Input:** "/review" right after building the voucher-link generation endpoint.

**Output:** Reads the latest `context/architecture.md` entry, `context/code-standards.md`, and the progress tracker. Then:

```
## Review — Voucher link generation

### Layer 1 — Plan alignment
ISSUES FOUND
- Plan specified a per-recipient cap of 5,000; code uses maxBatchSize (10,000). [Important]
- Plan's "log every mint" step is absent — no log line on the success path. [Minor]

### Layer 2 — System integrity
PASS
- Handler pattern, route registration, and error shapes match internal/notification.

### Layer 3 — Production readiness
ISSUES FOUND
- Upstream voucher-service timeout is unhandled — a slow mint returns a bare 500
  with no context for the operator. [Critical]

### Summary
3 issues found across 2 layers.
Resolve the above before moving to the next feature.
```

Then stops. Does not fix the timeout.

---

## Anti-patterns

- **Reviewing without a benchmark.** No plan, no feature description, no standards read — then everything "looks fine." If there's no plan, ask for one; don't invent the intent you're measuring against.
- **Fixing.** The most common failure: you find a bug and reflexively patch it. Report it and stop — the developer owns the call.
- **Softening.** "Might want to consider possibly handling…" A critical issue described politely reads as a minor one. State the defect plainly.
- **Padding to look thorough.** Filling a report with naming nitpicks buries the one issue that matters. If a layer is clean, say PASS and move on.
- **Claiming a layer passed without checking it.** If you didn't read the design tokens, don't mark Layer 2 PASS — say what you didn't check.
- **Drifting into /break's job.** Speculating about hostile inputs and race conditions is stress-testing, not review. Suggest running /break; don't half-do it here.
- **Reviewing the diff only.** A change can be correct in isolation and wrong in context — read the surrounding file and the pattern it should match.
