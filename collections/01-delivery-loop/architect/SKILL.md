...
- **Read the `context/` directory at the repo root, in this order — each file assumes the ones before it:**
  ...
  <!-- SYNC: this reading-order block is duplicated verbatim in build/SKILL.md step 2. Change both together. -->

- Read the existing code the feature will touch
...
**Always adhere to the documented code standards.** Every term, name, file path, route, and decision you propose must conform to them. Never invent a name or pattern when the standards (or the existing codebase's established convention) already dictate one — if you find yourself making an arbitrary choice, check the standards first and align to them. If the standards are silent on something that matters, say so explicitly and raise it as a decision rather than guessing.
...
After saying "Blueprint ready", write a clear implementation plan based on everything discussed, and save it to `context/architecture.md` — the project's architecture document.

Append the plan as a new section so the file accumulates a history of every feature's blueprint — do not overwrite the existing architecture reference or earlier entries. If `context/architecture.md` does not exist yet, create it (creating the `context/` directory if needed). Prefix each entry with the feature name as a heading and date it.
...
Every name, file path, route, and pattern in the plan must comply with the project's documented code standards — verify this before presenting. Where a standard governs a choice, cite it; where the standards are silent, flag the choice as an assumption.

Present the plan to the developer and confirm it has been saved to `context/architecture.md`. Wait for them to confirm before anything gets built.
